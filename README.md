...
## Business Problem
Dermatologists play a crucial role in assisting patients in regions with high UV exposure and a higher percentage of the elderly population. As a dermatologist, you will need to analyze the patient's skin lesions prior to their initial visit. By utilizing the software provided, you can input the image of the skin lesion and upload it to the patient's portal, along with the classification results. This valuable tool ensures that during the patient's visit, the proper course of action can be promptly determined based on the accurate diagnosis, aiding in early detection and timely treatment of potential skin conditions, including skin cancer.



## Why this project? 
According to one estimate, approximately 9,500 people in the U.S. are diagnosed with skin cancer daily. Skin cancer is also the most common cancer in the United States and is appearing increasingly in populations with excess exposure to UV radiation. An image classification model to detect certain types of skin diseases is useful in diagnosing and preparing to treat such a disease since it is easily accessible with the deployed application. Individuals without ready access to healthcare who are hesitant to seek medical help can use this project to test themselves and have the self-determination to decide if they are at risk. While it does not replace the need to see a doctor or dermatologist, it is a step in raising awareness to those who believe that a harmless freckle or mole is a non-threatening thing then their lives may be at risk. 
...
## Results:
My CNN-built model performed better than the transfer learning models. For example, the CNN accuracy reached 76% with an AUC score of 96%. The MobilenetV2 model which was going to be used in deployment only reached an accuracy of 71% and an AUC score of 94%. This could be due to poor engineering of the MobilenetV2 on my part and in future work I will improve on this model. 
...
## Difficutultes and future recommendations 
While there are over 10000 images to train, there is a huge imbalance in the data. For instance, melanocytic nevi appear about 6705 times while Dermatofibroma only appears 115 times. While image augmentation and standardization are used to fix this imbalance, I believe it drops the accuracy of the model dramatically. Scores when standardization was done before the test train split were done reached around 90% and higher. But this creates overfitting. When done properly, an accuracy of 76% was achievable using my CNN model. For future work, I will include external data to address this issue. 
...
Another issue was the number of classes used in the model. The data were tested on 7 different classes, and for future work, I will address each of these classes separately rather than all at once to improve accuracy.
